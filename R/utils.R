...
find_scale <- function(x) {
  m <- abs(max(x))
  i <- c(0,1e-12, 1e-9, 1e-6, 1e-3, 1e3, 1e6, 1e9, 1e12)
  x <- c(1e-0, 1e12, 1e9, 1e6, 1e3, 1e-3, 1e-6, 1e-9, 1e-12)
  names(i) <- c('', 'p','n','u','m', 'K', 'M', 'B', 'T')
  return(list(names(i)[findInterval(m, i)], x[findInterval(m, i)]))
}

#' Theme for ggplot2
#' @noRd
theme_afmstudio <- function() {
  theme(
...
#' Pretty numbers is sci-notation  ----
#' @noRd
sci_notation <- function(x, units = "m", sf = 2){
  absx <- abs(signif(x, sf))

  o_mag_x <- log10(absx)
  mags <- seq(from = -15, to = 15, by = 3)
  names <- c("f", "p", "n", "u", "m", "","k", "M", "G", "T", "P")

  o_mag <- max(which(o_mag_x > mags))
  prefix <- names[o_mag]
  pretty_quant <- sign(x) * absx / 10^mags[o_mag]
  pretty_units <- paste0(pretty_quant," ", prefix,units)
  return(pretty_units)
}

#' Draw a vertical line in plotly  ----
#' @noRd
