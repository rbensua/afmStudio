#' Create temporary data that can be used by afmStudio when run locally
#' @noRd
set_local_data <- function(x) {

  if (nzchar(Sys.getenv('AFMSTUDIO_DATA'))) {
    e <- Sys.getenv('AFMSTUDIO_DATA')
    cli::cli_abort(c(
      'Environment variable AFMSTUDIO_DATA must be empty.',
      'i' = 'Unset AFMSTUDIO_DATA before you run the app.',
      'x' = 'AFMSTUDIO_DATA is set with value {e}.'
    ))
  }
  ...
  if (inherits(x, 'data.frame')) {
    tmp <- tempfile()
    saveRDS(x, tmp)
    Sys.setenv('AFMSTUDIO_DATA' = tmp)
  } else {
    Sys.setenv('AFMSTUDIO_DATA' = '')
